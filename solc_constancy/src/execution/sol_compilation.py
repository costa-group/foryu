...
# Default Yul optimizer step sequence (solc's built-in default, made explicit so it can be
# reused as a baseline for occurrence-based before/after comparisons, e.g. isolating a single
# LiteralRematerialiser ('T') or Rematerialiser ('m') application).
DEFAULT_OPTIMIZER_SEQUENCE = (
    "dfDvulfnTUtnIfxa[r]EscLMVcul [j]Trpeulxa[r]cLvifMCTUca[r]LSsTFOtfDnca[r]"
    "IulcscCTUtvifMxscCTUt TOntnfDIulvifMjmul[jul] VcTOcul jmul:fDnTOcmu"
)


def get_yul_details(settings: Dict[str, Any]) -> Dict[str, Any]:
    """
    Navigates to settings.optimizer.details.yulDetails, creating any missing levels, and
    returns that dict so 'optimizerSteps' can be read/written in place.
    """
    optimizer = settings.setdefault("optimizer", {})
    optimizer.setdefault("enabled", True)
    details = optimizer.setdefault("details", {})
...
class SolidityCompilation:
    ...
    def __init__(self, final_file: Optional[str], solc_command: str):
        ...
        # Fixed Yul optimizer step sequence to force for this compilation, if any. Applied
        # either as settings.optimizer.details.yulDetails.optimizerSteps (standard-json mode)
        # or as the --yul-optimizations command-line flag (CLI mode)
        self._optimizer_steps: Optional[str] = None
    ...
    def set_optimizer_steps(self, steps: Optional[str]) -> None:
        ...
        whether the underlying invocation is standard-json or command-line based
        """
        self._optimizer_steps = steps

    def _optimizer_steps_flag(self) -> str:
        """
        Command-line flag that forces the Yul optimizer step sequence, if one was set
        """
        return f" --yul-optimizations '{self._optimizer_steps}'" if self._optimizer_steps is not None else ""
    ...
    def from_single_solidity_code(sol_file: str, deployed_contract: Optional[str] = None,
        ...
        """
        Compiles a single sol file. If optimizer_steps is given, it forces that Yul optimizer
        step sequence via the --yul-optimizations command-line flag
        """
    ...
    def compile_json_input(self, json_input: Dict, deployed_contract: Optional[str] = None,
        ...
        # Force a fixed Yul optimizer step sequence if one was requested
        if self._optimizer_steps is not None:
            get_yul_details(json_input["settings"])["optimizerSteps"] = self._optimizer_steps
    ...
    def _compile_sol_command(self, sol_file: str):
        command = f"{self._solc_command} {self.flags}{self._optimizer_steps_flag()} {sol_file}"
        output, error = run_command(command)
        return output, error
