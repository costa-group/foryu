...
def heavy_inlining_contract_input():
    ...
    function inlined at two different call sites. Used by src/constancy/occurrence_trace.py's
...
def multi_use_contract_input():
    ...
    different operators. Used by src/constancy/occurrence_trace.py's tests: same story as
...
def loop_carried_contract_input():
    ...
    `for` loop, never mutated. Used by src/constancy/occurrence_trace.py's tests: the 3rd "m"
